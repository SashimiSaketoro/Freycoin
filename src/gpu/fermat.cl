/**
 * OpenCL Fermat Primality Test Kernel for Freycoin
 *
 * Copyright (c) 2014 Jonny Frey <j0nn9.fr39@gmail.com>
 * Copyright (c) 2025 The Freycoin developers
 *
 * In memory of Jonnie Frey (1989-2017), creator of Gapcoin.
 *
 * This implements Montgomery modular exponentiation for large integers
 * (320-bit and 352-bit) used in Fermat primality testing.
 *
 * Key algorithms:
 * - Montgomery multiplication/squaring
 * - Window-based modular exponentiation
 * - Multi-limb arithmetic (32-bit limbs)
 */

/* Lookup table for Montgomery inverse */
__constant uint binvert_limb_table[128] = {
    
};

/**
 * Compute Montgomery inverse of lowest limb
 */
inline uint invert_limb(uint limb) {
    uint inv = binvert_limb_table[(limb >> 1) & 0x7F];
    inv = 2 * inv - inv * inv * limb;
    inv = 2 * inv - inv * inv * limb;
    return -inv;
}

/**
 * Subtraction with borrow propagation for 10 limbs
 */
inline void sub10(uint *a, __private uint *b) {
    uint A0 = a[0];
    a[0] -= b[0];

    for (int i = 1; i < 10; i++) {
        uint A1 = a[i];
        a[i] -= b[i] + (a[i-1] > A0 ? 1 : 0);
        A0 = A1;
    }
}

/**
 * Subtraction with borrow propagation for 11 limbs
 */
inline void sub11(uint *a, __private uint *b) {
    uint A0 = a[0];
    a[0] -= b[0];

    for (int i = 1; i < 11; i++) {
        uint A1 = a[i];
        a[i] -= b[i] + (a[i-1] > A0 ? 1 : 0);
        A0 = A1;
    }
}

/**
 * Montgomery squaring for 320-bit numbers (10 limbs)
 */
void monSqr320(__private uint *op, __private uint *mod, uint invm) {
    uint invValue[10];
    ulong accLow = 0, accHi = 0;

    /* Process each limb */
    for (int i = 0; i < 10; i++) {
        /* Diagonal term */
        ulong prod = (ulong)op[i] * op[i];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);

        /* Cross products (doubled) */
        for (int j = 0; j < i; j++) {
            prod = (ulong)op[j] * op[i];
            accLow += (uint)prod; accLow += (uint)prod;
            accHi += (uint)(prod >> 32); accHi += (uint)(prod >> 32);
        }

        /* Reduction terms */
        for (int j = 0; j < i; j++) {
            prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        invValue[i] = invm * (uint)accLow;
        prod = (ulong)invValue[i] * mod[0];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);

        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    /* Output phase */
    for (int i = 0; i < 9; i++) {
        /* Cross products */
        for (int j = i + 1; j < 10; j++) {
            int k = 10 + i - j;
            if (k >= 0 && k < 10 && k > j) {
                ulong prod = (ulong)op[j] * op[k];
                accLow += (uint)prod; accLow += (uint)prod;
                accHi += (uint)(prod >> 32); accHi += (uint)(prod >> 32);
            }
        }

        /* Diagonal */
        int diag = (10 + i) / 2;
        if ((10 + i) % 2 == 0 && diag < 10) {
            ulong prod = (ulong)op[diag] * op[diag];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        /* Reduction terms */
        for (int j = i + 1; j < 10; j++) {
            ulong prod = (ulong)invValue[j] * mod[10 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        op[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op[9] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub10(op, mod);
}

/**
 * Montgomery multiplication for 320-bit numbers (10 limbs)
 */
void monMul320(__private uint *op1, __private uint *op2, __private uint *mod, uint invm) {
    uint invValue[10];
    ulong accLow = 0, accHi = 0;

    /* First phase: compute invValues */
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j <= i; j++) {
            ulong prod = (ulong)op1[j] * op2[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        for (int j = 0; j < i; j++) {
            ulong prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        invValue[i] = invm * (uint)accLow;
        ulong prod = (ulong)invValue[i] * mod[0];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    /* Output phase */
    for (int i = 0; i < 9; i++) {
        for (int j = i + 1; j < 10; j++) {
            ulong prod = (ulong)op1[j] * op2[10 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        for (int j = i + 1; j < 10; j++) {
            ulong prod = (ulong)invValue[j] * mod[10 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        op1[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op1[9] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub10(op1, mod);
}

/**
 * Montgomery reduction for 320-bit numbers
 */
void redcHalf320(__private uint *op, __private uint *mod, uint invm) {
    uint invValue[10];
    ulong accLow = op[0], accHi = op[1];

    /* First reduction step */
    invValue[0] = invm * (uint)accLow;
    ulong prod = (ulong)invValue[0] * mod[0];
    accLow += (uint)prod;
    accHi += (uint)(prod >> 32);
    accHi += (uint)(accLow >> 32);
    accLow = accHi;
    accHi = op[2];

    /* Remaining reduction steps */
    for (int i = 1; i < 9; i++) {
        for (int j = 0; j < i; j++) {
            prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        invValue[i] = invm * (uint)accLow;
        prod = (ulong)invValue[i] * mod[0];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = op[i+2];
    }

    /* i = 9 */
    for (int j = 0; j < 9; j++) {
        prod = (ulong)invValue[j] * mod[9-j];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
    }
    invValue[9] = invm * (uint)accLow;
    prod = (ulong)invValue[9] * mod[0];
    accLow += (uint)prod;
    accHi += (uint)(prod >> 32);
    accHi += (uint)(accLow >> 32);
    accLow = accHi;
    accHi = 0;

    /* Output phase */
    for (int i = 0; i < 9; i++) {
        for (int j = i + 1; j < 10; j++) {
            prod = (ulong)invValue[j] * mod[10 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        op[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op[9] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub10(op, mod);
}

/**
 * Count leading zeros
 */
inline int count_leading_zeros(uint x) {
    int n = 0;
    if (x == 0) return 32;
    if ((x & 0xFFFF0000) == 0) { n += 16; x <<= 16; }
    if ((x & 0xFF000000) == 0) { n += 8; x <<= 8; }
    if ((x & 0xF0000000) == 0) { n += 4; x <<= 4; }
    if ((x & 0xC0000000) == 0) { n += 2; x <<= 2; }
    if ((x & 0x80000000) == 0) { n += 1; }
    return n;
}

/**
 * Fermat test for 320-bit number
 * Tests if 2^(p-1) = 1 (mod p)
 */
bool fermat320(__private uint *p) {
    uint mod[10], redcl[10], window[8][10];
    uint invm;
    int bitCount, windowSize = 3;  /* Smaller window for OpenCL */

    /* Copy modulus */
    for (int i = 0; i < 10; i++) mod[i] = p[i];

    /* Get Montgomery inverse */
    invm = invert_limb(mod[0]);

    /* Find highest bit */
    bitCount = 320;
    for (int i = 9; i >= 0; i--) {
        if (mod[i]) {
            int clz = count_leading_zeros(mod[i]);
            bitCount = i * 32 + (32 - clz);
            break;
        }
    }

    /* Initialize 2 in Montgomery form */
    for (int i = 0; i < 10; i++) redcl[i] = 0;
    redcl[0] = 2;

    /* Convert 2 to Montgomery form by left shifting and reducing */
    for (int i = 0; i < 320; i++) {
        uint carry = 0;
        for (int j = 0; j < 10; j++) {
            ulong tmp = ((ulong)redcl[j] << 1) | carry;
            redcl[j] = (uint)tmp;
            carry = (uint)(tmp >> 32);
        }
        if (carry || redcl[9] >= mod[9]) {
            int doSub = 1;
            for (int j = 9; j >= 0; j--) {
                if (redcl[j] > mod[j]) { doSub = 1; break; }
                if (redcl[j] < mod[j]) { doSub = 0; break; }
            }
            if (carry || doSub) sub10(redcl, mod);
        }
    }

    /* Build window table */
    for (int i = 0; i < 10; i++) window[1][i] = redcl[i];

    for (int i = 2; i < 8; i++) {
        for (int j = 0; j < 10; j++) window[i][j] = window[i-1][j];
        monMul320(window[i], window[1], mod, invm);
    }

    /* Initialize result to 1 in Montgomery form */
    uint exp[10];
    for (int i = 0; i < 10; i++) exp[i] = mod[i];
    exp[0] &= ~1u;  /* exp = p - 1 */

    for (int i = 0; i < 10; i++) redcl[i] = 0;
    redcl[0] = 1;
    for (int i = 0; i < 320; i++) {
        uint carry = 0;
        for (int j = 0; j < 10; j++) {
            ulong tmp = ((ulong)redcl[j] << 1) | carry;
            redcl[j] = (uint)tmp;
            carry = (uint)(tmp >> 32);
        }
        if (carry) sub10(redcl, mod);
        int doSub = 0;
        for (int j = 9; j >= 0; j--) {
            if (redcl[j] > mod[j]) { doSub = 1; break; }
            if (redcl[j] < mod[j]) break;
        }
        if (doSub) sub10(redcl, mod);
    }

    /* Main exponentiation loop — process ALL bitCount bits from MSB to LSB */
    int remaining = bitCount;
    while (remaining > 0) {
        int bits = (remaining >= windowSize) ? windowSize : remaining;
        int start = remaining - bits;

        /* Extract window value from exponent */
        int wordIdx = start / 32;
        int bitIdx = start % 32;
        uint windowVal;
        if (bitIdx + bits <= 32) {
            windowVal = (exp[wordIdx] >> bitIdx) & ((1u << bits) - 1);
        } else {
            windowVal = (exp[wordIdx] >> bitIdx) |
                       ((exp[wordIdx + 1] << (32 - bitIdx)) & ((1u << bits) - 1));
        }

        /* Square 'bits' times */
        for (int i = 0; i < bits; i++) {
            monSqr320(redcl, mod, invm);
        }

        /* Multiply by window value if non-zero */
        if (windowVal) {
            monMul320(redcl, window[windowVal], mod, invm);
        }

        remaining = start;
    }

    /* Convert back from Montgomery form */
    redcHalf320(redcl, mod, invm);

    /* Check if result is 1 */
    if (redcl[0] != 1) return false;
    for (int i = 1; i < 10; i++) {
        if (redcl[i] != 0) return false;
    }
    return true;
}

/**
 * OpenCL Kernel: Batch Fermat test for 320-bit numbers
 */
__kernel void fermat_kernel_320(__global uchar *results,
                                 __global const uint *primes,
                                 uint count) {
    uint id = get_global_id(0);
    if (id >= count) return;

    uint p[10];
    for (int i = 0; i < 10; i++) {
        p[i] = primes[id * 10 + i];
    }

    results[id] = fermat320(p) ? 1 : 0;
}

/**
 * Montgomery squaring for 352-bit numbers (11 limbs)
 */
void monSqr352(__private uint *op, __private uint *mod, uint invm) {
    uint invValue[11];
    ulong accLow = 0, accHi = 0;

    /* Process each limb */
    for (int i = 0; i <= 10; i++) {
        /* Diagonal term */
        if (i <= 10) {
            ulong prod = (ulong)op[i] * op[i];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        /* Cross products (doubled) */
        for (int j = 0; j < (i+1)/2; j++) {
            int k = i - j;
            if (k <= 10 && k != j) {
                ulong prod = (ulong)op[j] * op[k];
                accLow += (uint)prod; accLow += (uint)prod;
                accHi += (uint)(prod >> 32); accHi += (uint)(prod >> 32);
            }
        }

        /* Reduction terms */
        for (int j = 0; j < i; j++) {
            ulong prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        if (i <= 10) {
            invValue[i] = invm * (uint)accLow;
            ulong prod = (ulong)invValue[i] * mod[0];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    /* Output phase */
    for (int i = 0; i < 10; i++) {
        /* Cross products */
        for (int j = i + 1; j <= 10; j++) {
            int k = 11 + i - j;
            if (k >= 0 && k <= 10 && k > j) {
                ulong prod = (ulong)op[j] * op[k];
                accLow += (uint)prod; accLow += (uint)prod;
                accHi += (uint)(prod >> 32); accHi += (uint)(prod >> 32);
            }
        }

        /* Diagonal */
        int diag = (11 + i) / 2;
        if ((11 + i) % 2 == 0 && diag <= 10) {
            ulong prod = (ulong)op[diag] * op[diag];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        /* Reduction terms */
        for (int j = i + 1; j <= 10; j++) {
            ulong prod = (ulong)invValue[j] * mod[11 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }

        op[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op[10] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub11(op, mod);
}

/**
 * Montgomery multiplication for 352-bit numbers (11 limbs)
 */
void monMul352(__private uint *op1, __private uint *op2, __private uint *mod, uint invm) {
    uint invValue[11];
    ulong accLow = 0, accHi = 0;

    /* First phase: compute invValues */
    for (int i = 0; i <= 10; i++) {
        for (int j = 0; j <= i; j++) {
            ulong prod = (ulong)op1[j] * op2[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        for (int j = 0; j < i; j++) {
            ulong prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        invValue[i] = invm * (uint)accLow;
        ulong prod = (ulong)invValue[i] * mod[0];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    /* Output phase */
    for (int i = 0; i < 10; i++) {
        for (int j = i + 1; j <= 10; j++) {
            ulong prod = (ulong)op1[j] * op2[11 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        for (int j = i + 1; j <= 10; j++) {
            ulong prod = (ulong)invValue[j] * mod[11 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        op1[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op1[10] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub11(op1, mod);
}

/**
 * Montgomery reduction for 352-bit numbers
 */
void redcHalf352(__private uint *op, __private uint *mod, uint invm) {
    uint invValue[11];
    ulong accLow = op[0], accHi = op[1];

    /* First step */
    invValue[0] = invm * (uint)accLow;
    ulong prod = (ulong)invValue[0] * mod[0];
    accLow += (uint)prod;
    accHi += (uint)(prod >> 32);
    accHi += (uint)(accLow >> 32);
    accLow = accHi;
    accHi = op[2];

    /* Remaining steps */
    for (int i = 1; i < 10; i++) {
        for (int j = 0; j < i; j++) {
            prod = (ulong)invValue[j] * mod[i-j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        invValue[i] = invm * (uint)accLow;
        prod = (ulong)invValue[i] * mod[0];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = op[i+2];
    }

    /* i = 10 */
    for (int j = 0; j < 10; j++) {
        prod = (ulong)invValue[j] * mod[10-j];
        accLow += (uint)prod;
        accHi += (uint)(prod >> 32);
    }
    invValue[10] = invm * (uint)accLow;
    prod = (ulong)invValue[10] * mod[0];
    accLow += (uint)prod;
    accHi += (uint)(prod >> 32);
    accHi += (uint)(accLow >> 32);
    accLow = accHi;
    accHi = 0;

    /* Output phase */
    for (int i = 0; i < 10; i++) {
        for (int j = i + 1; j <= 10; j++) {
            prod = (ulong)invValue[j] * mod[11 + i - j];
            accLow += (uint)prod;
            accHi += (uint)(prod >> 32);
        }
        op[i] = (uint)accLow;
        accHi += (uint)(accLow >> 32);
        accLow = accHi;
        accHi = 0;
    }

    op[10] = (uint)accLow;
    if ((uint)(accLow >> 32))
        sub11(op, mod);
}

/**
 * Fermat test for 352-bit number
 */
bool fermat352(__private uint *p) {
    uint mod[11], redcl[11], window[8][11];
    uint invm;
    int bitCount, windowSize = 3;

    for (int i = 0; i < 11; i++) mod[i] = p[i];

    invm = invert_limb(mod[0]);

    bitCount = 352;
    for (int i = 10; i >= 0; i--) {
        if (mod[i]) {
            int clz = count_leading_zeros(mod[i]);
            bitCount = i * 32 + (32 - clz);
            break;
        }
    }

    /* Initialize 2 in Montgomery form */
    for (int i = 0; i < 11; i++) redcl[i] = 0;
    redcl[0] = 2;

    for (int i = 0; i < 352; i++) {
        uint carry = 0;
        for (int j = 0; j < 11; j++) {
            ulong tmp = ((ulong)redcl[j] << 1) | carry;
            redcl[j] = (uint)tmp;
            carry = (uint)(tmp >> 32);
        }
        if (carry) sub11(redcl, mod);
        int doSub = 0;
        for (int j = 10; j >= 0; j--) {
            if (redcl[j] > mod[j]) { doSub = 1; break; }
            if (redcl[j] < mod[j]) break;
        }
        if (doSub) sub11(redcl, mod);
    }

    /* Build window table */
    for (int i = 0; i < 11; i++) window[1][i] = redcl[i];

    for (int i = 2; i < 8; i++) {
        for (int j = 0; j < 11; j++) window[i][j] = window[i-1][j];
        monMul352(window[i], window[1], mod, invm);
    }

    /* Initialize result */
    uint exp[11];
    for (int i = 0; i < 11; i++) exp[i] = mod[i];
    exp[0] &= ~1u;

    for (int i = 0; i < 11; i++) redcl[i] = 0;
    redcl[0] = 1;
    for (int i = 0; i < 352; i++) {
        uint carry = 0;
        for (int j = 0; j < 11; j++) {
            ulong tmp = ((ulong)redcl[j] << 1) | carry;
            redcl[j] = (uint)tmp;
            carry = (uint)(tmp >> 32);
        }
        if (carry) sub11(redcl, mod);
        int doSub = 0;
        for (int j = 10; j >= 0; j--) {
            if (redcl[j] > mod[j]) { doSub = 1; break; }
            if (redcl[j] < mod[j]) break;
        }
        if (doSub) sub11(redcl, mod);
    }

    /* Main exponentiation loop — process ALL bitCount bits from MSB to LSB */
    int remaining = bitCount;
    while (remaining > 0) {
        int bits = (remaining >= windowSize) ? windowSize : remaining;
        int start = remaining - bits;

        int wordIdx = start / 32;
        int bitIdx = start % 32;
        uint windowVal;
        if (bitIdx + bits <= 32) {
            windowVal = (exp[wordIdx] >> bitIdx) & ((1u << bits) - 1);
        } else {
            windowVal = (exp[wordIdx] >> bitIdx) |
                       ((exp[wordIdx + 1] << (32 - bitIdx)) & ((1u << bits) - 1));
        }

        for (int i = 0; i < bits; i++) {
            monSqr352(redcl, mod, invm);
        }

        if (windowVal) {
            monMul352(redcl, window[windowVal], mod, invm);
        }

        remaining = start;
    }

    redcHalf352(redcl, mod, invm);

    if (redcl[0] != 1) return false;
    for (int i = 1; i < 11; i++) {
        if (redcl[i] != 0) return false;
    }
    return true;
}

/**
 * OpenCL Kernel: Batch Fermat test for 352-bit numbers
 */
__kernel void fermat_kernel_352(__global uchar *results,
                                 __global const uint *primes,
                                 uint count) {
    uint id = get_global_id(0);
    if (id >= count) return;

    uint p[11];
    for (int i = 0; i < 11; i++) {
        p[i] = primes[id * 11 + i];
    }

    results[id] = fermat352(p) ? 1 : 0;
}
