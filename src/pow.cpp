...
double GetDifficulty(uint32_t nBits, const int32_t powVersion)
{
    if (powVersion == -1)
    {
        ...
        return mpz_get_d(difficulty.get_mpz_t());
    }
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight + 1 >= params.fork2Height) {
        ...
        if (pindexLast->nHeight + 1 == params.fork2Height) { // Take previous Difficulty/1.5, which is arbitrary, but approximates well enough the corresponding Difficulty for the transition from k to k + 1 tuples.
            arith_uint256 difficultyAu256;
            difficultyAu256.SetCompact(pindexLast->nBits);
            if (difficultyAu256 > 25116767) difficultyAu256 = 25116767;
            nBits = difficultyAu256.GetLow64()*171; // In the new format, the nBits is Difficulty/256, and 2*256/3 = ~171
...
mpz_class GenerateTarget(mpz_class &target, uint256 hash, unsigned int nBits, const int32_t powVersion)
{
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, uint256 nOffset, const Consensus::Params& params)
{
    ...
    if ((nOffset.GetUint64(0) & 1) == 1)
    {
        ...
        if (fNegative || dummy < UintToArith256(params.powLimit) || fOverflow)
            return false;
        // The Block Validation (see validation.cpp) consists of Contextual (including the PoW Version) and Non Contextual verifications. In Bitcoin, CheckProofOfWork is Non Contextual, and it is not clear if Contextual Checks are always done before CheckProofOfWork (making the PoW Version check in Non Contextual code would require to rewrite and test many things, which we would like to avoid until more Riecoin Core Developers join). So for now, do a sanity check in case someone finds a way to force to check blocks with PoW Version -1 as Version 1 after the Second Fork, as they would be interpreted as huge Difficulties and freeze nodes. At worst the block would pass CheckProofOfWork without slowing down too much then be rejected later. It is hugely unlikely that the Difficulty reaches this before the Fork.
        if (dummy > 3072)
            return false;
    }
    ...
    mpz_class target, trailingZeros, offset, offsetLimit(1);
