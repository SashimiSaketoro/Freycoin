...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //riecoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Riecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Riecoin
    // Mac: ~/Library/Application Support/Riecoin
    // Unix: ~/.riecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Riecoin";
#else
    fs::path pathRet;
...
{
    std::string strCopyrightHolders = strPrefix + " 2009-2020 The Bitcoin Core developers";
    strCopyrightHolders += "\n" + strPrefix + " 2013-2020 The Riecoin developers";
