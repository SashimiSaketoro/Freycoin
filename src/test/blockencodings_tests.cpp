...
static CBlock BuildBlockTestCase() {
    ...
    assert(!mutated);
    block.nNonce = 1;
    while (!CheckProofOfWork(block.GetHashForPoW(), block.nBits, ArithToUint256(block.nNonce), Params().GetConsensus())) block.nNonce += 2;
    return block;
}
...
BOOST_AUTO_TEST_CASE(EmptyBlockRoundTripTest)
{
    ...
    block.hashMerkleRoot = BlockMerkleRoot(block, &mutated);
    assert(!mutated);
    while (!CheckProofOfWork(block.GetHashForPoW(), block.nBits, ArithToUint256(block.nNonce), Params().GetConsensus())) ++block.nNonce;
