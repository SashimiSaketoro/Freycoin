...
/* Test the constraint on the lower bound for next work */
...
{
    const auto consensus = CreateChainParams(CBaseChainParams::REGTEST)->GetConsensus();
    uint256 hash, offset;
    unsigned int nBits(UintToArith256(consensus.powLimit).GetCompact(true));
    hash.SetHex("0x00");
...
BOOST_AUTO_TEST_CASE(CheckProofOfWork_test_too_easy_target)
{
    const auto consensus = CreateChainParams(CBaseChainParams::REGTEST)->GetConsensus();
    uint256 hash, offset;
    unsigned int nBits(arith_uint256(UintToArith256(consensus.powLimit) - 1).GetCompact());
    hash.SetHex("0x00");
