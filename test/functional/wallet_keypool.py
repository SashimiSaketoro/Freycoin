...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-4, "Transaction needs a change address, but we can't generate it.", w2.walletcreatefundedpsbt, inputs=[], outputs=[{addr.pop(): 0.00005000}], options={"subtractFeeFromOutputs": [0], "fee_rate": 10})
        ...
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00010000}], options={"subtractFeeFromOutputs": [0], "fee_rate": 88.23})
        assert_equal("psbt" in res, True)
